...
//-----------------------------------------------------------------------------
HRESULT CWaveFile::Open(const LPTSTR strFileName, WAVEFORMATEX* pwfx, DWORD dwFlags)
{
    HRESULT hr;
    ...
    else
    {
        ...
        else
        {
            ...
            mmioinfo.fccIOProc = FOURCC_MEM;
            mmioinfo.cchBuffer = 2048 * 1024;
            memset(&m_hmmio, 0, sizeof(m_hmmio));
            m_hmmio = mmioOpen(NULL, &mmioinfo, MMIO_READWRITE | MMIO_CREATE);
            if (NULL == m_hmmio)
                return DXTRACE_ERR(TEXT("mmioOpen"), E_FAIL);
            ...
            mmioGetInfo(m_hmmio, &mmioinfo, 0);
            m_pbData = (BYTE*)mmioinfo.pchBuffer;
            m_pbDataCur = m_pbData;
            m_ulDataSize = mmioinfo.cchBuffer;
...
HRESULT CWaveFile::WriteMMIO(WAVEFORMATEX* pwfxDest)
{
    ...
    MMRESULT mr = mmioCreateChunk(m_hmmio, &m_ckRiff, MMIO_CREATERIFF);
    if (0 != mr)
        return DXTRACE_ERR(TEXT("mmioCreateChunk"), E_FAIL);
